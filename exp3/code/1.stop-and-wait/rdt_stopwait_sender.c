...
int deliver_file(char *send_file_name, int sockfd) {
    ...
    while (1) {
        ...
        while (1) {
            ...
            if(poll(&pollfd,1,RDT_TIME_OUT)) { 
                int num,temp,len;
                char recv_rdt_pkt[RDT_PKT_MAX_LEN];
                len=recv(sockfd,recv_rdt_pkt,RDT_PKT_MAX_LEN,0);        
                unpack_rdt_pkt(rdt_data,recv_rdt_pkt,len,&num,&temp);
                if(num==seq_num&&temp==RDT_CTRL_ACK){printf("[Sender]Receive ACK#%d\n", num);break;}
                else continue;
            }
            else{printf("FLAG\n");continue;}           
        ...
        pkt_len = pack_rdt_pkt(rdt_data, rdt_pkt, data_len, seq_num, flag);
    }

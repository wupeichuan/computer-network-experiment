        ...
        if (flag == RDT_CTRL_ACK) {
            ...
            if (seq >= send_window.left && seq < send_window.right) {
                for (int i = send_window.left; i <= seq; i++) {
                    ...
                    printf("\t[child thread]modify flag #pkt %-8d #state%-8d\n", i,send_window.rdt_pkts[i % send_window.len].state);
...
int prepare_send_window(FILE *fp) {
    ...
    // step 1. 如果滑动窗口最左端的包已经收到ACK，则将滑动窗口滑动到下一个没收到ACK的包的位置。
        while(1){
            if(send_window.left==send_window.right) break;     
            else if(ptr_pkt_left->state==RDT_PKT_ST_ACKED){ 
            send_window.left++;
            ptr_pkt_left = &send_window.rdt_pkts[send_window.left % send_window.len];
            }
            else {break;}
        }
    // step 2. 将新数据包装入滑动窗口右    if(data_len>0){端空缺位置。
    int pkt_to_send = 0; // 需要发送的数据包个数
    while((send_window.right-send_window.left)<send_window.len){
    char rdt_data[RDT_DATA_MAX_LEN];
    if(feof(fp)){break;}
    else{
    int data_len= fread(rdt_data, sizeof(char), RDT_DATA_MAX_LEN, fp);
    StatePkt *ptr_pkt = &send_window.rdt_pkts[send_window.right% send_window.len];
    ptr_pkt->pkt_len=pack_rdt_pkt(rdt_data,ptr_pkt->rdt_pkt, data_len, send_window.right,RDT_CTRL_DATA);
    ptr_pkt->state=RDT_PKT_ST_INIT;
    ptr_pkt->pkt_seq=send_window.right;
    send_window.right++;
    pkt_to_send++;
